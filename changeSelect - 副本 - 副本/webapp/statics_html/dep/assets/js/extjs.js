...
$.fn.dataTableExt.oApi.fnExtStylePagingInfo = function ( oSettings )
{
	return {
		...
		"iTotalPages":    oSettings._iDisplayLength === -1 ?
			0 : Math.ceil( oSettings.fnRecordsDisplay() / 2 )
	};
};
...
$.fn.dataTableExt.oPagination.extStyle = {
    ...
    "fnInit": function (oSettings, nPaging, fnCallbackDraw) {
        ...
        nFirst = $('<span/>', { 'class': 'paginate_button btn btn-minier btn-info first' , text : "首页" , 'style':'margin-right:3px'});
        nPrevious = $('<span/>', { 'class': 'paginate_button btn btn-minier btn-info previous' , text : "上一页" , 'style':'margin-right:3px'});
        nNext = $('<span/>', { 'class': 'paginate_button btn btn-minier btn-info next' , text : "下一页" , 'style':'margin-right:3px;margin-left:3px'});
        nLast = $('<span/>', { 'class': 'paginate_button btn btn-minier btn-info last' , text : "尾页" });
        nPageTxt = $("<span />", { text: '页' , 'style':'margin-left:3px'});
        nPageNumBox = $('<input />', { type: 'text', val: 1, 'class': 'pageinate_input_box input-sm' , 'style':'width:7%;height: 25px;text-align:center; vertical-align:middle'});
        nPageOf = $('<span />', { text: '/' , 'style':'margin-right:3px;margin-left:3px'});
