...
//	      url: ctx + '/consultInfoManage/findProductOptionNew',
	      ...
	      success: function (data) {
	          if (data.status == "success") {
            	 ...
            	 //回显上一步中选中的产品
            	 $("#productIdCTB").val($("#updateInfoManageCTB").find("select.productId").val());
            	 //加载下拉框样式
            	 $("#productIdCTB").trigger('chosen:updated');
            	 $('.chosen-container').width('100%');
            	 //触发产品的change事件，生成考期等其它信息
            	 baoProductChange($("#productIdCTB"));
	          } else {
...
//	productId = clearProductCTB(false);
	clearProductCTB(false);
	productId = $("#updateInfoManageCTB").find("select.productId").val();
	//得到选中的option的Json信息
	...
	if(null==jsonObj ||　typeof(jsonObj)=="undefined" || jsonObj.length==0) {
		...
		for(var i=0; i<jsonObj.length; i++) {
			...
			//开始拼接 
			var str = '<div class="form-group col-md-4 col-sm-4">'
				...
				+ '            <select data-id="'+enName+'" id="'+enName+'" onchange="clearProductCTB(true)" class="form-control removeFlag productInfo chosen-select" data-live-search="true" disabled>'
...
//	}
	...
	if(productId!=null && productId!='' && typeof(productId)!="undefined") {
		...
		$(".addPayFees").parents(".form").append('<div class="well with-header addChildProject">'
								...
								+ ' 	<label class="col-sm-4 col-md-4 col-lg-4 control-label no-padding-right" style="margin-left:-41px">产品类型'
...
function baoProductChange() {
	...
	//得到产品信息Json格式
	var showList = $("#updateInfoManageCTB").find("select.productId").find(":selected").attr("showList");
...
//				url : ctx + '/consultConsoleWFC/getChildrenProduct',
				...
				data : {productId: productId,"addressId":departmentId,"studentPhone":studentPhone},//将主产品id作为参数传递过去
				success : function(data){
					//如果主产品下没有查到子产品
					if(data.status!="success") {
						var str = '<div class="form-group col-lg-4 col-md-4 col-sm-4 childrenProductCTB">'
		                    + '       <label class="control-label col-sm-4 col-md-4 col-lg-4 no-padding-right" style="margin-left:-41px">'+"子产品"+'</label>'
		                    + '       <div class="col-sm-9 col-md-9 col-lg-9 no-padding-right">'
...
function baokTimeChange(obj) {
    ...
    appendPayDivCTB(examTimeId,'addPayInfo', productId);
}
...
function branchSchoolChangeCTB() {
	...
	var examTimeIdStr = $("#kTimeCTB").val();//产品考期id
        	...
        	if(data2.status=='success'){
        		...
        		if(data2.ZKList!=null && typeof(data2.ZKList)!='undefined' && data2.ZKList.length>0) {
        			$("#branchSchoolId").parent().parent().after(
   		               ...
   		               + '   <div class="col-sm-9 col-lg-9 col-lg-9 no-padding-right">'
