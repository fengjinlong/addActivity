//子产品产品模型初始化
function genChildModelCTB(ProductDetail) {
	//获得子产品的产品模型id
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#childProductModelIdCTB").html('<option value="">--请选择--</option>' + zxkc);
            $('#childProductModelIdCTB').trigger('chosen:updated');
            $("#childProductModelIdCTB").chosen({no_results_text: "没有匹配项", search_contains: true});
            $('.chosen-container').width('100%');
             
            //产品模型初始化完成后，触发一次铲平模型的change事件，为了实现其它下拉框的回显。
//            $("#childProductModelId").trigger("change");
            ChildProModelChangeCTB();
        },
...
function ChildProModelChangeCTB() {
	//清空考期下拉选
	$('#childKTimeCTB').html('');
	$('#childKTimeCTB').trigger('chosen:updated');
	$("#childKTimeCTB").chosen({no_results_text: "没有匹配项", search_contains: true});
	//产品模型发生改变，制空产品下拉框,如果没有制空则会得到初始时的productId
	var productId = "";
	productId = $("#childProductIdCTB").val();
	//得到选中的option的Json信息
	var jsonObj = $('#childProductModelIdCTB :selected').data("value");
	//如果当前模型下没有配置选项
	if(jsonObj=="undefined"||　typeof(jsonObj)=="undefined" || jsonObj.length==0) {
		//清除上次选择后生成的下拉框
    	$(".addChildProject").find(".removeChildFlag").parent().parent().remove();
		return;
	}
	
	//得到产品类型ID
	var modelId = $('#childProductModelIdCTB :selected').val();
	//清除上次选择后生成的下拉框
	$(".addChildProject").find(".removeChildFlag").parent().parent().remove();
	//用来组装表名
	...
	if(null==jsonObj ||　typeof(jsonObj)=="undefined" || jsonObj.length==0) {
		...
		for(var i=0; i<jsonObj.length; i++) {
			...
			//开始拼接 
			var str = '<div class="form-group col-md-4 col-sm-4 col-lg-4">'
				+ '       <label class="control-label col-sm-4 col-md-4 col-lg-4 no-padding-right" style="margin-left:-41px">'+chName+'</label>'
				+ '       <div class="col-sm-9 col-md-9 col-lg-9 no-padding-right">'
				+ '            <select name="childrenProduct.projectMap[\''+enName+'_id\']" id="child_'+enName+'CTB" class="form-control removeChildFlag chosen-select" disabled data-live-search="true">'
				+ '            </select>'
				+ '			   <input type="hidden" name="childrenProduct.projectMap[\''+enName+'_name\']" class="projectInfoManager2" />'		
				+ '        </div>'
				+ '</div>';
			//将新增的下拉框拼接到产品类型下拉框后面
			$("#childProductModelIdCTB").parent().parent().after(str);
...
//    	        $("#childProductId").trigger("change");
    	        childPro2ChangeCTB($("#childProductIdCTB"));
            	return;
            }
            for (var i = 0; i < data.length; i++) {
            	var zxkc = "";
            	...
            	$('#child_'+data[i].tableName+"CTB").html('<option value="">--请选择--</option>' + zxkc);
            	//加载下拉框样式
            	$('#child_'+data[i].tableName+"CTB").trigger('chosen:updated');
            	$("#child_"+data[i].tableName+"CTB").chosen({no_results_text: "没有匹配项", search_contains: true});
            	$('.chosen-container').width('100%');
            }
            //根据产品id回显其它课程信息，生成考期选项-考期是后面新增的下拉选
//	        $("#childProductId").trigger("change");
...
function childPro2ChangeCTB(obj) {
	...
	if(showList!=null){
		...
		for(var p in showList){
			aiId = p.replace("_id","");
			$('#child_'+aiId+"CTB").val(showList[p]);
			$('#child_'+aiId+"CTB").trigger('chosen:updated');
		}
	}
	 
	//将其它课程信息值为空的select去掉
	$(".addChildProject select.removeChildFlag").each(function(index,obj){
	...
	var productId = $('#childProductIdCTB').val();
	//咨询量id
	...
	$.ajax({
		...
		success : function(map){
			...
			$("#childPayeeCTB").html(option);
			$('#childPayeeCTB').trigger('chosen:updated');
			$("#childPayeeCTB").chosen({no_results_text: "没有匹配项", search_contains: true});
		}
	});
	...
	//得到产品考期信息 
	var departmentId = $("#updateInfoManageCTB").find("select.departmentId1").val();//信息量归属地id-分校id
	//得到产品考期信息 
	$.ajax({
		...
		success : function(info){
			if (info == null || info.length == 0){
				$('#childKTimeCTB').html('<option value="">--请选择--<option/>');
				$('#childKTimeCTB').trigger('chosen:updated');
				$("#childKTimeCTB").chosen({no_results_text: "没有匹配项", search_contains: true});
//				return;
			} else {
				...
				$('#childKTimeCTB').html('<option value="">--请选择--<option/>'+exm);
				$('#childKTimeCTB').trigger('chosen:updated');
				$("#childKTimeCTB").chosen({no_results_text: "没有匹配项", search_contains: true});
			}
...
//	        $('#childKTime').trigger('change');
			//调用考期change事件
	        childProExamChangeCTB($("#childKTimeCTB"));
	      ...
	      //得到产品考试地区信息 
	    	$.ajax({
	    		...
	    		success : function(msg){
	    			...
	    			$("#childBranchSchoolIdCTB").html(option);
	    			$('#childBranchSchoolIdCTB').trigger('chosen:updated');
	    			$("#childBranchSchoolIdCTB").chosen({no_results_text: "没有匹配项", search_contains: true});
	    			//触发考试地区的change事件-构造支出费用与合计(第一次初始化页面，考试地区回显时要调用一次）
	    			childBranchSchoolChangeCTB();
	    		}
	    	});
...
//产品考期change事件
function childProExamChangeCTB(obj) {
    ...
    //生成子产品课程缴费信息模块--拼接到子课程信息父表单后面
    //$(".addChildProject").after('<div class="well with-header addChildPayFees">'
    $(".addChildProject").parents(".form").append('<div class="well with-header addChildPayFees">'
					...
					+'					<th>欠费</th>'
					+'				</tr>'
					+'			<tbody id="appendChildPayBodyCTB" >'
					+'			</tbody>'
					+'			<tr id="appendChildPayTrCTB">'
					+'				<td colspan="6">'
					+'					<div class="form-group col-lg-12 col-md-12 col-sm-12">'
		            +'                        <label class="col-sm-2  no-padding control-label">下次缴费时间：</label>'
		            +'                        <div class="input-group col-sm-8 payment-time"> '
		            +'                            <input name="childrenProduct.nextPayTime" id="childNextPayTimeCTB" class="form-control paymentTime" type="text">'
					...
					+'	</div>');
    //生成缴费信息
    appendChildPayDivCTB(examTimeId,'appendChildPayBodyCTB',productId);
...
function childBranchSchoolChangeCTB() {
	//清空以前生成的支出费用
	$("#appendChildPayBodyCTB").find("tr.zhiChu").remove();
	var branchSchoolIdStr = $("#childBranchSchoolIdCTB").val();//考试地区id
	var productExamTimeIdStr = $("#childKTimeCTB").val();//产品考期id
	var productIdStr = $("#childProductIdCTB").val();//产品id
	//清除子产品课程信息下以前生成的积分，折扣，优惠码
	$(".addChildProject .yhzkjf").remove();
        ...
        "success": function (data) {
        	if(data.status=='success'){
        		for(var i=0; i<data.list.length; i++) {
        				//拼接费用行
        				$('#childHjDivCTB').before('<tr class="appendDiv zhiChu">'
        						...
        						+ '</div>'
        						+ '<div class="col-sm-5">'
        						+ '     <input ondblclick="childDbclickCTB(this)" class="form-control zjsjflag" onkeyup="childSshjCTB(this)" type="text" placeholder="0" >'
        						+ '	    <input type="hidden" name="childPayFees.payZhiChuList['+i+'][payValue&Form]" class="payValueForm" value=""/>'
        						+ ' </div>'
        						+ '<div class="col-sm-2">'
        						+ '<i onclick="addChildRowPayCTB(this)" data-index="'+i+'" class="fa  fa-plus-circle payment-btn blue control-label"></i>'
        						+ '</div>'
        						+ '</div>'
        						+ '</td>'
        						+ '<td class="zjcfflag">'
        ...
        "success": function (data2) {
        	if(data2.status=='success'){
        		...
        		if(data2.ZKList!=null && typeof(data2.ZKList)!='undefined' && data2.ZKList.length>0) {
        			$("#childBranchSchoolIdCTB").parent().parent().after(
   		               ...
   		               + '   <div class="col-sm-9 col-md-9 col-lg-9 no-padding-right">'
   		               + '       <select id="childZkSelectCTB" name="discount" onchange="childZkChangeCTB(this)" class="form-control chosen-select">'
   		               + '       </select>'
   		               + '   </div>'
   		               + '</div>');
  	        	//拼接折扣下拉选-value:活动id，data-value：费用类型code_id集合，data-discount:折扣，data-multi：是否可叠加
      	        	...
      	        	$("#childZkSelectCTB").html(zkOption);
      	        	$('#childZkSelectCTB').trigger('chosen:updated');
      	        	$("#childZkSelectCTB").chosen({no_results_text: "没有匹配项", search_contains: true});
        		...
        		if(data2.JFList!=null && typeof(data2.JFList)!='undefined' && data2.JFList.length>0) {
        			$("#childBranchSchoolIdCTB").parent().parent().after(
   	               ...
   	               + '   <div class="col-sm-9 col-md-9 col-lg-9 no-padding-right">'
   	               + '       <select id="childJfSelectCTB" name="scale" onchange="childJfChangeCTB(this)" class="form-control chosen-select">'
   	               + '       </select>'
   	               + '   </div>'
   	               + '</div>');
        		//拼接积分下拉选-value:活动id，data-value：费用类型code_id集合，data-sclae1:元(积分兑换比例)，data-scale2:积分(积分兑换比例), data-multi：是否可叠加
   	        	...
   	        	$("#childJfSelectCTB").html(jfOption);
   	        	$('#childJfSelectCTB').trigger('chosen:updated');
   	        	$("#childJfSelectCTB").chosen({no_results_text: "没有匹配项", search_contains: true});
        		}
        		...
        		if(data2.YHList!=null && typeof(data2.YHList)!='undefined' && data2.YHList.length>0) {
    				...
    				$("#childBranchSchoolIdCTB").parent().parent().after(
    						...
    						+ '   <div class="col-sm-9 col-md-9 col-lg-9 no-padding-right">'
    						+ '       <input id="childYhInputCTB" name="childPayComp.activityCode" onblur="childActivtyCodeCTB(this)" data-value="'+data2.YHList[0].isMulti+'" data-limit="'+data2.YHList[0].limit1+'" class="form-control childYhInput" placeholder="请输入8位优惠码"/>'
    						+ '       <input id="childYhqpriceCTB" name="childPayComp.activityValue" type="hidden" class="form-control childYhqprice"/>'
    						+ '   </div>'
    						+ '</div>');
        			//}
        		}
