...
$('#subjectAdd').bootstrapValidator({
    submitHandler: function (validator, form, submitButton) {
        ...
        $.ajax({
            "type": "Post",
            "url": ctx + "/bizExamSubject/addNewRecord",
            "dataType": "json",
            "data": options,
    ...
    return {
        init: function () {
            var dutyTable = $('#moneyKind').dataTable({
                ...
                "sAjaxSource": ctx + '/bizExamSubject/load',
                ...
                "aoColumns": [
                    {"mData": "examSubjectName", 'sClass': "text-center"},
                    {
                        "mData": "examSubjectId",
                        'sClass': "text-center",
                        "bSortable": false,
                        "mRender": function (data, type, full) {
                            var u = '<a onclick="edit(\'' + full["examSubjectId"]
                                + '\',\'' + full["projectType"]
                                + '\',\'' + full["examSubjectName"]
...
//编辑取值
function edit(examSubjectId, projectType, examSubjectName) {
    $('#category').val(projectType);
    $('#examSubject').val(examSubjectName);
    $('#examSubjectIds').val(examSubjectId);
}
//编辑修改
$('#subjectEdit').bootstrapValidator({
    submitHandler: function (validator, form, submitButton) {
        var examSubjectId = $('#examSubjectIds').val();
        var projectType = $('#category').val();
        var examSubjectName = $('#examSubject').val();
        $.ajax({
            "type": "Post",
            "url": ctx + "/bizExamSubject/updateRecord",
            "dataType": "json",
            "data": {examSubjectId: examSubjectId, projectType: projectType, examSubjectName: examSubjectName},
            "success": function (data) {
