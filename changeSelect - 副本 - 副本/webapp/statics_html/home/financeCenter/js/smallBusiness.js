$(function () {
    ...
    $("#init tbody").html("<tr><td height='300' colspan='21' class='text-center'></td></tr>");
...
function retrieveData(sSource, aoData, fnCallback, oSettings ){
	...
	aoData.push( { "name": "month", "value": $("#reservation").val()} );
...
function init (){
	var init = $('#init').dataTable({
		...
		"sAjaxSource" : ctx+'/smallBusiness/load',
		"bDestroy" : true,
		"bRetrieve" : false,
		"bServerSide" : true,
		...
		"aoColumns" : [
			...
			{"mData": "month", 'sClass': "text-center"},
			
			{"mDataProp" : "heji","bSortable": false,'sClass': "text-center"},
			{"mDataProp" : "jituan","bSortable": false,'sClass': "text-center"},
			{"mDataProp" : "fenxiao","bSortable": false,'sClass': "text-center","mRender":function(data,type,full){
					return Number(data).toFixed(2);
			}},
			{"mDataProp" : "zc","bSortable": false,'sClass': "text-center","mRender":function(data,type,full){
				return Number(data).toFixed(2);
			}},
			{"mDataProp" : "afterMoney","bSortable": false,'sClass': "text-center","mRender":function(data,type,full){
				if(data == '' ){
					return 0;
				}else{
					return Number(data).toFixed(2);
			...
			{"mDataProp" : "fc","bSortable": false,'sClass': "text-center","mRender":function(data,type,full){
				if(data == '' && data == null){
					return 0;
				}else{
					return data;
			...
			{"mDataProp" : "beforeMoney","bSortable": false,'sClass': "text-center","mRender":function(data,type,full){
				if(data == '' && data != null){
					return 0;
				}else{
					return data;
			...
			{"mDataProp" : "fenXiaoFuDan","bSortable": false,'sClass': "text-center","mRender":function(data,type,full){
				if(data == ''){
					return 0;
				}else{
					return data;
			...
			{"mDataProp" : "ds","bSortable": false,'sClass': "text-center","mRender":function(data,type,full){
				if(data == ''){
					return 0;
				}else{
					return data;
			...
			{"mDataProp" : "df","bSortable": false,'sClass': "text-center"},
			{"mDataProp" : "cf","bSortable": false,'sClass': "text-center","mRender":function(data,type,full){
				if(full["brandName"] == "1"){
			...
			{"mDataProp" : "jl","bSortable": false,'sClass': "text-center","mRender":function(data,type,full){
				if(full["brandName"] == "1"){
			...
			{"mDataProp" : "","bSortable": false,'sClass': "text-center","mRender": function ( data, type, full ) {
				var str = full["depId"]+','+full["month"]+','+full["brandName"];
				 var s1 = '<a onclick=loadcheck(\'smallBusiness/indexCheck/'+str+'\') data-record=\'' + JSON.stringify(full) + '\' href="javascript:void(0);" class="btn btn-warning btn-xs view"><i class="fa fa-folder-open-o"></i> 查看</a>';
				 var s2 = '<a href="#" data-record=\'' + JSON.stringify(full) + '\'  class="apply" data-toggle="modal" data-target=".financeApply" data-backdrop="static"><i class="fa fa-edit blue" data-toggle="tooltip" data-placement="top" data-original-title="申请" title="申请"></i></a>';
				if(full['cname']){
					return 	s1;
				}else{
					return s1+s2;
...
$("#init").on('click','.apply',function(){
	...
	$('#money').val(Number(record.fenxiao).toFixed(2));
...
function addApply(){
	...
	$.ajax({
        url: ctx + "/smallBusiness/addApply",
        type: 'POST',
        data: data,
        dataType: "json",
        success: function (data) {
