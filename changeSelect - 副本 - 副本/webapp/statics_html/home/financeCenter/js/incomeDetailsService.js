...
$(function () {
  ...
  //点击休学按钮
    $('.xiuxue').on('click',function(){
    	if(UNACTION==1||UNACTION==2||UNACTION==3||UNACTION==7){
...
function xiuxue(){
	$.ajax({
        ...
        dataType: 'json',
        data: {infoManageId:INFOID,productId:PROID},
        success: function (data) {
            if (data.status == 'success') {
...
function tuichajia(){
	...
	consultInfoManageBlankReturn1["memo"]=$("#kongzhichu [name='rebuildRemark']").val();
	...
	if($("#tuichajiaType").val()==1){
		...
		consultInfoManageBlankReturn2["money"]=$("#tuichajia input[name='kongzhichuMoney']").val();
		...
		consultInfoManageBlankReturn2["memo"]=$("#tuichajia [name='rebuildRemark']").val();
...
function fuwuFun(){
	...
	$.each($(".fuwuCheck"),function(index,obj){
		if($(obj).is(":checked")){
			...
			console.log(fuwu)
			str+='<tr class="fuwu">'
				+'<th scope="row">'+fuwu.expensesTypeName+'</th>'
				+'<td>'+fuwu.money+'</td>'
				+'<td>'+fuwu.money+'</td>'
...
function youhuiFun(val){
	if($(val).is(":checked")){//说明是点击选中
		...
		console.log($(".youhuiCheck").length)
		if($(".youhuiCheck").length==1){//只选中一个优惠
			youhuiCheck(youhui);
			return ;
		}else{//选中的第二个以后的优惠
