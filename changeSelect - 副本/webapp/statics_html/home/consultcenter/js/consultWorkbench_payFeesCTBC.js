function appendChildPayDivCTB(examTimeId, parentId, productId) {
	//生成费用列表前先把报名时生成回显的费用列表清空-防止生成的标签的id冲突
    $('#appendChildPayBodyCTB').html("");
        ...
        "success": function (data) {
        	if(data.status=='success'){
        		for(var i=0; i<data.list.length; i++) {
        		 ...
        		 //循环结束，拼接合计
          		 $('#appendChildPayBodyCTB').append('<tr id="childHjDivCTB">'
           	            + ' <td>合计</td>'
           	            + '<td id="childHjYJCTB">0</td>' //合计产品应缴
           	            + '<td id="childHjpriceCTB">0</td>' //合计实际应缴
           	            + '<td class="sjhj" id="childSjhjCTB">0</td>' //合计实缴
           	            + ' <td><div class="payment">'
           	            + '<div class="col-sm-4">'
           	            + '</div>'
           	            + '<div class="col-sm-5">'
        		...
        		$(".addChildProject .serviceFees").remove();
        		//生成服务费复选框
        		$.ajax({
        	        "type": "Post",
        	        	...
        	        	if(data3.status=='success'){
        	        		...
        	        		for(var x=0; x<data3.serviceList.length; x++) {
        	        			...
        	        			if(data3.serviceList[x].isRequired=="1") {
        	        				...
        	        				tempStr += '<div class="checkbox col-sm-2 col-lg-2 no-padding-right no-padding-left">'
        	                			+ '	 <label>'
        	        					+ '	    <input type="checkbox" onclick="childServiceCliCTB(this)" name="childService" class="childService" data-codeid="'+data3.serviceList[x].expensesTypeId+'" data-required="'+data3.serviceList[x].isRequired+'"'
        	        					+ '            data-money="'+data3.serviceList[x].money+'" data-moneyline="'+data3.serviceList[x].moneyLine+'" data-enable="'+data3.serviceList[x].serviceEnable+'"' 
        	        					+ '            data-code="'+data3.serviceList[x].expensesTypeCode+'" data-name="'+data3.serviceList[x].expensesTypeName+'" data-type="'+data3.serviceList[x].expensesType+'"'
        	        					+ '			   data-value="'+data3.serviceList[x].productServiceId+'">'
        	        					+ '		<span class="text">'+data3.serviceList[x].productServiceName+'</span>'		
        	        		...
        	        		tempStr += '</div>';
        	        		$("#childBranchSchoolIdCTB").parent().parent().after(tempStr);
        	        		
        	        		//遍历页面上必选的服务，把服务费用拼接到后面
        	        		$('.addChildProject').find("input[type='checkbox'].childService").each(function(i,e){
        	        			var required = $(e).data("required");//得到该服务是否必选
        	        			//判断是否必选1-是 0-否，如果是，该服务费默认勾选，并且不允许更改
        	        			if(required=='1') {
        	        				//是必选
        	        				//触发服务类型复选框单击事件
        	        				childServiceCliCTB(e);
        	        			}
        	        		});
        	        	}
        	        },
...
function childZkChangeCTB(obj) {
	...
	if(isMulti=="0") {//独立
		//使积分下拉选失效,并且使选项清空
		$("#childJfSelectCTB").val('');
		$("#childJfSelectCTB").prop("disabled",true);
		$('#childJfSelectCTB').trigger('chosen:updated');
		//清空优惠码输入框，并使优惠码输入框失效
		$(".addChildProject .childYhInput").each(function(index,obj){
			$(obj).val('');
			$(obj).prop("disabled",true);
		});
		//清空优惠码金额隐藏input金额
		$(".addChildProject .childYhqprice").val(0);
	} else if(isMulti=="1") {//可叠加
		//取消积分下拉选失效
		$("#childJfSelectCTB").prop("disabled",false);
		$('#childJfSelectCTB').trigger('chosen:updated');
		//取消优惠码输入框失效
		$(".addChildProject .childYhInput").each(function(index,obj){
	...
	var newValue = $("#childZkSelectCTB").val();
	$("#childZkSelectCTB").val('');//优惠不可用，将该选中优惠的值制为空
	$('#childZkSelectCTB').trigger('chosen:updated');
	childFlushYHZKCTB();//消除之前选中的折扣的效果-更新页面
	//再将本次选中的值重新设置回去（此时的页面上的金额就是正常的金额）
	$("#childZkSelectCTB").val(newValue);
	$('#childZkSelectCTB').trigger('chosen:updated');
	...
	if(useFlag==false) {
		var zkName = $("#childZkSelectCTB").val();
		$("#childZkSelectCTB").val('');//优惠不可用，将该选中优惠的值制为空
		$('#childZkSelectCTB').trigger('chosen:updated');
...
function childFlushYHZKCTB() {
//	toastr.error("更新优惠");
	//更新页面实际应缴金额前，先将页面上所有使用了优惠活动的费用（实际应缴小于产品应缴）
	//的实际应缴，实缴，支付方式，欠费整理还原，方便下面的计算
	$(".addChildPayFees tr.appendDiv").each(function(index,obj){
		//得到产品应缴
		...
		if(Number(yjValue)<Number(proValue)) {
			...
			$(obj).find('td').eq(4).html('<div class="payment addPayment">'
			        ...
			        + '  </div>'
			        + '  <div class="col-sm-5">'
			        + '     <input class="form-control zjsjflag" ondblclick="childDbclickCTB(this)" isat="1" onkeyup="childSshjCTB(this)" type="text" placeholder="0" >'
			        + '	    <input type="hidden" name="childPayFees.payList['+indexCount+'][payValue&Form]" class="payValueForm" value=""/>'
			        + ' </div>'
			        + '<div class="col-sm-3">'
			        + '<i onclick="addChildRowPayCTB(this)" data-index="'+indexCount+'" class="fa  fa-plus-circle payment-btn blue control-label"></i>'
			        + '</div>'
			        + '</div>');//还原支付方式
			//最后还原欠费
	...
	//根据页面已选中的折扣更新实际应缴金额
	//先得到页面选中的折扣
	var zkValue = $("#childZkSelectCTB").val();
	if(zkValue!=null && zkValue!='') {//折扣有可用值选中
		//得到该折扣活动的code_id集合
		var epId = $("#childZkSelectCTB").find(":selected").data("value");
		epId = epId.toString();//确保得到的是字符串
		if(epId=='' || epId==null) {//如果epId为空串
			var epIdArr = new Array();
		} else if(epId.indexOf(",")==-1) {//如果epId集合中只有一个code_id
		...
		$(".addChildPayFees tr.appendDiv").each(function(i,e){
			...
			for(var index in epIdArr) {
				if(codeId==epIdArr[index]) {//当前费用种类的id在code_id集合中，更新该费用的实际应缴和欠费
					...
					var proValue = $(e).find("td").eq(1).text();
					//得到当前折扣比例
					var discount = $("#childZkSelectCTB").find(":selected").data("discount");
					//得到当前实际应缴值
					var yjValue = $(e).find("td").eq(2).text();
					//得到当前折扣金额
					...
					//设置折扣活动id
					$(e).find("input.discount").val($("#childZkSelectCTB").val());
					//设置折扣金额
	...
	//根据页面已选中的优惠码更新实际应缴金额
	//得到页面选中的优惠码
	var yhValue = $("#childYhInputCTB").val();
	if(yhValue!=null && yhValue!='') {//优惠码有可用值填写
		//更新实际应缴，欠费
		//得到收益类型费用价格最大的对象
		var firstObj = null;
		$('#appendChildPayBodyCTB').find('tr.appendDiv').each(function(i,e){
			//取费用类型-是2:收益还是1:支出
		...
		//得到优惠金额
		var yhMoney = Number($('#childYhqpriceCTB').val());//优惠金额
		//得到实际应缴金额
		...
		//设置优惠码活动id
		$(firstObj).find("input.activityCode").val($("#childYhInputCTB").val());
		//设置优惠码金额
		$(firstObj).find("input.activityValue").val($("#childYhqpriceCTB").val());
	}
    ...
    if (e.value.length == 8) {
    	...
    	if(codeObj!=null && typeof(codeObj)!="undefined") {
    		...
    		if(payCode==codeStr) {
            	...
            	//取消折扣下拉选失效
        		$("#childZkSelectCTB").prop("disabled",false);
        		$('#childZkSelectCTB').trigger('chosen:updated');
        		//取消积分下拉选失效
        		$("#childJfSelectCTB").prop("disabled",false);
        		$('#childJfSelectCTB').trigger('chosen:updated');
...
function childBeforeYHCTB(obj) {
	//1.先判断该优惠是否可叠加
	var isMulti = $(obj).data("value");
	//0-独立，1-可叠加
	if(isMulti=="0") {//独立
		//使折扣下拉选失效,并且使选项清空
		$("#childZkSelectCTB").val('');
		$("#childZkSelectCTB").prop("disabled",true);
		$('#childZkSelectCTB').trigger('chosen:updated');
		//使积分下拉选失效,并且使选项清空
		$("#childJfSelectCTB").val('');
		$("#childJfSelectCTB").prop("disabled",true);
...
//		toastr.error("没有收益类型费用，不能使用优惠");
	...
	if(serviceEnable=='1') {//正常
		childYhCalcuNewCTB(obj,firstObj);//调用新版优惠折扣算法-只适用于正常状态（下限无效，统一为0）
	} else {//serviceEnable=='0' 锁定
		childYhCalcuCTB(obj,firstObj);//调用以前的优惠折扣算法-只适用于锁定状态（下限有效）
...
function childYhCalcuNewCTB(obj,firstObj) {
	var trObj = $(firstObj);
 ...
 //得到优惠码的下限
  ...
  if(Number(preferential)<0) {
   ...
   //计算总计，合计的应缴，实缴，欠费
   //yjCalcu();
	childFlushYHZKCTB();//实际更新页面金额
}

//用于锁定状态
//优惠部分复选框单击事件-优惠部分的计算，只将金额计算到第一个费用上，不能超过金额下限
function childYhCalcuCTB(obj,firstObj) {
	var trObj = $(firstObj);
...
function childServiceCliCTB(obj) {
	...
	var indexCount = $("#appendChildPayBodyCTB").find("tr.appendDiv").length-$("#appendChildPayBodyCTB").find("tr.zhiChu").length;
	...
	$('#childHjDivCTB').before('<tr class="appendDiv" id="child'+service+'CTB">'
			+ ' <td>' + name + '</td>'
			+ '<td>' 
			+  price //产品应缴
			+ '</td>'
...
//    var hj = $('#childHjprice').text();//合计实际应缴
    ...
    $('#appendChildPayBodyCTB').find('tr.appendDiv').each(function (i,e) {
        ...
        //剔除实缴合计的td对象
        if ($(jlid).attr('id') != "childSjhjCTB" ) {
            lss = jlid.text();
            sjhj = sjhj + eval((lss != null && lss != '') ? lss : 0);
        }
        if ($(cfid).attr('id') != "childCfhjCTB") {
            lss = cfid.text();
        ...
        //剔除实缴合计的td对象
        if ($(proYJtd).attr('id') != "childHjYJCTB" ) {
            lss = proYJtd.text();
            proYJhj = proYJhj + eval((lss != null && lss != '') ? lss : 0);
        }
        if ($(sjYJtd).attr('id') != "childHjpriceCTB") {
            lss = sjYJtd.text();
            sjYJhj = sjYJhj + eval((lss != null && lss != '') ? lss : 0);
        }
    })
    $('#childHjYJCTB').text(proYJhj);//合计产品应缴
    $('#childHjpriceCTB').text(sjYJhj);//合计实际应缴
    $('#childSjhjCTB').text(sjhj);//合计实缴
    $('#childCfhjCTB').text(cfhj);//合计欠费结果
    //欠费合计-控制下次缴费时间的展示
    if (cfhj > 0) {
        $('#appendChildPayTrCTB').show();
    } else {
        $('#appendChildPayTrCTB').hide();
    }
}

//转报名-费用部分相关处理js-添加新的缴费方式(有可能一次付款使用多种方式付款)-只适用于子产品
function addChildRowPayCTB(e) {
	...
	if($(e).hasClass("zhiChu")) {//是支出类型费用
		 $(e).parent().parent().parent().append('<div class="payment addPayment">'
			        ...
			        + '  </div>'
			        + '  <div class="col-sm-5">'
			        + '     <input ondblclick="childDbclickCTB(this)" class="form-control zjsjflag" isat="1" onkeyup="childSshjCTB(this)" type="text" placeholder="0" >'
			        + '	    <input type="hidden" name="childPayFees.payZhiChuList['+list1Index+'][payValue&Form]" class="payValueForm" value=""/>'
			        + ' </div>'
			        + '<div class="col-sm-3">'
			        + '  <i onclick="removeRowPayCTB(this)" data-index="'+list1Index+'" class="fa fa-minus-circle danger control-label"></i>'
			        + '</div>'
			        + '</div>');
	} else {
		 $(e).parent().parent().parent().append('<div class="payment addPayment">'
