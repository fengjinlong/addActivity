...
$('#kTime').chosen().change(function () {
    ...
    var productExamTimeId = $(this).find(':selected').val();//产品考期ID
    //得到数据查询出来时的状态值
   ...
   //只有当前处于6-订座,才会生成缴费列表
    ...
    //$('#classPrice2').val('');//将前次合计制空
//    $('#selfYH2').val(0);
//    $('#ortherYH2').val(0);
   // appendPayDiv(obj.attr("priceList"), obj.attr("examFee"));//priceList：费用列表，examFee：考务费
    //$('#activtyCode2').val('');//activtyCode-优惠码制空
});
//根据选中的考期，自动生成拼接费用选项
        	...
        	if(data.status=='success'){
        		...
        		$('#'+parentId).append('<tr>'
        	        ...
        	        $('#'+parentId).append('<tr>'
        	            ...
        	            + '</div>'
        	            + '<div class="col-sm-5">'
        	            + ' <input class="form-control" onkeyup="activtyCode(this,'+id+')" type="text" placeholder="请输入8位优惠码" >'
        	            + ' </div>'
        	            + '<div class="col-sm-3">'
        	            + '</div>'
        	            + '</div>'
...
//    	$(e).parent().find("input.payValue").val($(e).val());
//    	$(e).parent().find("input.payForm").val($(e).parent().parent().find("select :selected").val());
    }
}
...
//	var list2Index = $(e).parent().parent().parent().find(".addPayment").length;
//	list2Index += 1;
    $(e).parent().parent().parent().append('<div class="payment addPayment">'
		...
		//+ '	    <input type="hidden" name="payList['+list1Index+'][payForm]" class="payForm" value=""/>'
        + ' </div>'
        + '<div class="col-sm-3">'
	...
	if(td=='appendPayTd1'){//如果该td是报名费的td
		var price = $(e).parents('tr').children('td').eq(1).text();//每行应缴部分金额
		   if(dprice!=null){//如果订座费不为空
			   //$(e).parents('tr').children('td').eq(2).text(price - dprice);//报名费部分实缴金额
			   $(e).parents('tr').children('td').eq(2).text(price);//报名费部分实缴金额
			   //$(e).val(price - dprice);//报名部分支付方式当前操作框金额
