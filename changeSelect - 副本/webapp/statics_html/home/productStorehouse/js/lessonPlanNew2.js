...
function openUploadW(obj) {
	...
	var str = '<div class="modal fade '+objId+'Win in" tabindex="-1" role="dialog"                                                       '
			...
			+'				<input type="file" name="myfile" class="form-control coursewareContent uploadFile" data-bv-field="myfile"/>  '
...
//			+	'		<input type="hidden" id="upload'+randNum+'FileName" name="detailList[0].courseware" class="form-control courseware uploadFileName" data-bv-field="detailList[0].courseware"/>                                                                                '
			+	'	</div>                                                                                                                                                                                         '
			+	'</td>                                                                                                                                                                                             '
			+	'<td>                                                                                                                                                                                              '
			+	'	<div class="col-sm-12" style="padding:0">                                                                                                                                                                        '
...
function uploadFile() {
	$(".uploadFile").fileinput({
	    //上传的地址
	    language: 'zh', //设置语言
        uploadUrl: ctx + "/proLessonPlan/uploadFile", //上传的地址
            ...
            var dutyTable = $('#moneyKind').dataTable({
                ...
                "aoColumns": [
                    {"mData": "lessonPlan.departmentName", 'sClass': "text-center","mRender": function (data, type, full) {
                    	...
                    	return '<p style="max-width: 200px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" title="'+data+'">'+data+'</p>';
                    }},
                    ...
                    {"mData": "lessonPlan.productName", 'sClass': "text-center","mRender": function (data, type, full) {
                    	...
                    	return '<p style="max-width: 100px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" title="'+data+'">'+data+'</p>';
                    }},
...
//$('#editProductModel').val(productModelId);
//$('#editProduct').val(productId);
//$('#editProductExamTime').val(productExamTimeId);
//展示所有单元选项
$('#subjectEdit .lessonPlanUnitId').html(lessonPlanUnitList);
//回显单元信息
$('#subjectEdit .lessonPlanUnitId').val(lessonPlanUnitId);
//回显单元名称
$('#subjectEdit .lessonPlanUnitName').val(lessonPlanUnitName);
//$('#subjectEdit .lessonPlanUnitId').trigger('chosen:updated');
//$('#subjectEdit .lessonPlanUnitId').chosen({no_results_text: "没有匹配项", search_contains: true});
//$('.chosen-container').width('100%');
//展示所有教室选项
$('#subjectEdit .schoolRoomId').html(schoolRoomList);
//回显教室信息
$('#subjectEdit .schoolRoomId').val(schoolRoomId);
//回显教室名称
$('#subjectEdit .schoolRoomName').val(schoolRoomName);
//$('#subjectEdit .schoolRoomId').trigger('chosen:updated');
//$('#subjectEdit .schoolRoomId').chosen({no_results_text: "没有匹配项", search_contains: true});
//$('.chosen-container').width('100%');
//展示所有教师选项
$('#subjectEdit .teacherId').html(teacherList);
//回显教师信息
$('#subjectEdit .teacherId').val(teacherId);
//回显教师名称
$('#subjectEdit .teacherName').val(teacherName);
//$('#subjectEdit .teacherId').trigger('chosen:updated');
//$('#subjectEdit .teacherId').chosen({no_results_text: "没有匹配项", search_contains: true});
//$('.chosen-container').width('100%');
...
////        		str += '<option value="'+data[i].departmentId+'" selected>'+data[i].shortName+'</option>';
////    		} else {
////        		str += '<option value="'+data[i].departmentId+'">'+data[i].shortName+'</option>';
////    		}
//    		str += '<option value="'+data[i].departmentId+'">'+data[i].shortName+'</option>';
//    	}
//    	$("#editDepartment").html(str);
////    	$("#editDepartment").trigger('chosen:updated');
////    	$("#editDepartment").chosen({no_results_text: "没有匹配项", search_contains: true});
//// 	    $('.chosen-container').width('100%');
//    	$('.selectpicker').selectpicker('refresh');
//    	if(departmentId!=null && departmentId!='') {
...
//    	}
//    	//回显部门
//    	$('#editDepartment').selectpicker('val', departmentId);
//    }
//});
	...
	//回显产品单元
		$.ajax({
			...
			success : function(info){
				...
				$("#subjectEdit").find("select[name='lessonPlanUnitId']").html(str);
//				$("#subjectAdd").find("select[name='lessonPlanUnitId']").trigger('chosen:updated');
//	     	    $("#subjectAdd").find("select[name='lessonPlanUnitId']").chosen({no_results_text: "没有匹配项", search_contains: true});
//	     	    $('.chosen-container').width('100%');
		     	 lessonPlanUnitList = str;
	...
	$.ajax({
		...
		success : function(info){
			...
			$("#subjectEdit").find("select[name='schoolRoomId']").html(str);
//			$("#subjectAdd").find("select[name='schoolRoomId']").trigger('chosen:updated');
//     	    $("#subjectAdd").find("select[name='schoolRoomId']").chosen({no_results_text: "没有匹配项", search_contains: true});
//     	    $('.chosen-container').width('100%');
     	   schoolRoomList = str;
	...
	$.ajax({
		...
		success : function(info){
			...
			$("#subjectEdit").find("select[name='teacherId']").html(str);
//			$("#subjectAdd").find("select[name='teacherId']").trigger('chosen:updated');
//     	    $("#subjectAdd").find("select[name='teacherId']").chosen({no_results_text: "没有匹配项", search_contains: true});
//     	    $('.chosen-container').width('100%');
     	   teacherList = str;
...
function addRow(obj) {
	...
	var str = '<tr>                                                                                                                                          '
			+	'<td>                                                                                                                                          '
			+	'	<div class="col-sm-12 setName" style="pading:0">                                                                                                                    '
			...
			+	'</td>                                                                                                                                         '
			+	'<td>                                                                                                                                          '
			+	'	<div class="col-sm-12" style="pading:0">                                                                                                                    '
			...
			+	'<td>                                                                                                                                          '
			+	'	<div class="col-sm-12" style="pading:0">                                                                                                                    '
			+	'		<input type="file" name="myfile" class="form-control myfile uploadFile" data-bv-field="myfile"/>                                                                                       '
