...
$('#subjectAdd').bootstrapValidator({
    submitHandler: function (validator, form, submitButton) {
        ...
        $.ajax({
            "type": "Post",
            "url": ctx + "/bizTestTermController/addRecord",
            "dataType": "json",
            "data": options,
    ...
    return {
        init: function () {
            var dutyTable = $('#moneyKind').dataTable({
                ...
                "sAjaxSource": ctx + '/bizTestTermController/load',
                ...
                "aoColumns": [
                    {"mData": "termName", 'sClass': "text-center"},
                    {
                        "mData": "termId",
                        'sClass': "text-center",
                        "bSortable": false,
                        "mRender": function (data, type, full) {
                            var u = '<a onclick="edit(\'' + full["termId"]
                                + '\',\'' + full["termName"]
                            ...
                            var d = '<a onclick="deleteProject(\'' + full["termId"] + '\')" class="delete"> <i class="fa fa-trash-o danger"  data-toggle="tooltip" data-placement="top" data-original-title="删除" title="删除"></i></a>';
                            return u + d;
                        }
                    }
...
function retrieveData(sSource, aoData, fnCallback, oSettings) {
    ...
    aoData.push({"name": "termName", "value": searchVal});
...
$('#subjectEdit').bootstrapValidator({
    submitHandler: function (validator, form, submitButton) {
        var termId = $('#termIds').val();
        var termName = $('#termName').val();
        $.ajax({
            "type": "Post",
            "url": ctx + "/bizTestTermController/updateRecord",
            "dataType": "json",
            "data": {termId: termId, termName: termName},
            "success": function (data) {
