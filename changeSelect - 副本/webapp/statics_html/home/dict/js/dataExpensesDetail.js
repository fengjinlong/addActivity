...
$('#subjectAdd').bootstrapValidator({
    submitHandler: function (validator, form, submitButton) {
        ...
        var str = $('#subjectAdd input[name="dataExpensesDetailName"]').val();
        console.info(str)
        ...
        $.ajax({
            "type": "Post",
            "url": ctx + "/dataExpensesDetail/addRecord",
            "dataType": "json",
            "data": options,
...
function chooseStudent(val, flag) {
    ...
    $.ajax({
        url: ctx + '/dataExpensesDetail/updateRecord',
        type: 'POST',
        data: {
        	dataExpensesDetailId: val,
            enable: flag
        },
        dataType: 'json',
        success: function (data) {
    ...
    return {
        init: function () {
            var dutyTable = $('#moneyKind').dataTable({
                ...
                "sAjaxSource": ctx + '/dataExpensesDetail/load',
                ...
                "aoColumns": [
                    {"mData": "dataExpensesDetailName", 'sClass': "text-center"},
                     
                    {"mData": "enable", 'sClass': "text-center","bSortable": false,"mRender":function(data, type, full ){
		            	if(data==0){
		          		  return '<span style="width: inherit" id="span'+full["dataExpensesDetailId"]+'" onclick="chooseStudent(\''+full["dataExpensesDetailId"]+'\')" class="btn btn-xs btn-nouse"><i class="fa fa-ban"></i> 禁用</span>';
		          	  }else{
		          		  return '<span style="width: inherit" id="span'+full["dataExpensesDetailId"]+'" onclick="chooseStudent(\''+full["dataExpensesDetailId"]+'\')" class="btn btn-xs btn-use"><i class="fa fa-check-circle-o"></i> 启用</span>';
		          	  }
		            }},
                    {
                        "mData": "productTypeId",
                        ...
                        "mRender": function (data, type, full) {
                            var u = '<a onclick="edit(\'' + full["dataExpensesDetailId"]
                                + '\',\'' + full["dataExpensesDetailName"]
                            	+ '\',\'' + inCode(full["memo"])
...
//编辑取值
function edit(dataExpensesDetailId, dataExpensesDetailName,memo) {
    $('#dataExpensesDetailName').val(dataExpensesDetailName);
    $('#dataExpensesDetailId').val(dataExpensesDetailId);
    $('#memo').val(outCode(memo));
}
//编辑修改
$('#subjectEdit').bootstrapValidator({
    submitHandler: function (validator, form, submitButton) {
        var dataExpensesDetailId = $('#dataExpensesDetailId').val();
        var dataExpensesDetailName = $('#dataExpensesDetailName').val();
        var memo = $('#memo').val();
        $.ajax({
            "type": "Post",
            "url": ctx + "/dataExpensesDetail/updateRecord",
            "dataType": "json",
            "data": {
            	dataExpensesDetailId: dataExpensesDetailId, 
            	dataExpensesDetailName: dataExpensesDetailName,
            	memo: memo
            },
            "success": function (data) {
