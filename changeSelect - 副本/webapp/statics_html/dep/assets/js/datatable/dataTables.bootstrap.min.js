...
$.extend($.fn.dataTableExt.oPagination, {
    bootstrap: {
        fnInit: function (n, t, i) {
            var u = n.oLanguage.oPaginate, f = function (t) {
                t.preventDefault();
                n.oApi._fnPageChange(n, t.data.action) && i(n)
            }, r;
            $(t).append('' +
                '<div class="jumpPage pull-right">'
                + '<span class="jumpPage-text">到</span>'
                + '<input type="text" class="form-control pageSize"><span class="jumpPage-text">页</span>'
                + '<button class="btn btn-azure paging-button">确定</button>'
                + '</div>'
                + '<ul class="pagination"><li class="prev disabled"><a href="#">' + u.sPrevious + '<\/a><\/li><li class="next disabled"><a href="#">' + u.sNext + "<\/a><\/li><\/ul>");
            r = $("a", t);

            //datatables分页跳转
            //$(t).find(".pageSize").keyup(function(e){
            //    var ipage = parseInt($(this).val());
            //    var oPaging = n.oInstance.fnPagingInfo();
            ...
            //    $(this).val(ipage);
            //    ipage--;
            //    n._iDisplayStart = ipage * oPaging.iLength;
            //    i( n );
            //});
            ...
            $(t).find(".pageSize").keyup(function (e) {
                ...
                if (regex.test($(this).val())) {
                    $(this).val($(this).val());
                }else if($(this).val() == 'undefiend' || $(this).val() == ''){
                    $(this).val('');
                }else{
                    $(this).val(1);
                }
            })
            $(t).find(".paging-button").click(function (e) {
                var ipage = parseInt($(this).siblings('.pageSize').val());
                var oPaging = n.oInstance.fnPagingInfo();
                if (isNaN(ipage) || ipage < 1) {
                    ipage = 1;
                } else if (ipage > oPaging.iTotalPages) {
                ...
                $(this).siblings('.pageSize').val(ipage);
                ipage--;
                n._iDisplayStart = ipage * oPaging.iLength;
                i(n);
            });

            $(r[0]).bind("click.DT", {action: "previous"}, f);
            $(r[1]).bind("click.DT", {action: "next"}, f)
        }, fnUpdate: function (n, t) {
            var e = 5, i = n.oInstance.fnPagingInfo(), u = n.aanFeatures.p, r, c, o, l, f, s, h = Math.floor(e / 2);
