//构造缴费信息-拼接收益费用
function appendPayDivCTB(examTimeId, parentId, productId) {
	...
	$.ajax({
        ...
        "success": function (data) {
        	if(data.status=='success'){
        		for(var i=0; i<data.list.length; i++) {
        			...
        			if(i==0) {
            				...
            				$('#'+parentId).prepend('<tr class="appendDiv firstDiv">'
            						...
            						+ '<td data-id="appendPayTd1" data-value="'+data.list[i].moneyLine+'" data-value2="'+data.list[i].price+'" data-value3="'+data.list[i].serviceEnable+'" data-value4="'+data.list[i].expensesType+'">' 
        				...
        				$('#'+parentId).append('<tr class="appendDiv">'
        						...
        						+ data.list[i].price //产品应缴
        						+ '</td>'
        						+ '<td data-id="' + data.list[i].code + '" data-value="'+data.list[i].moneyLine+'" data-value2="'+data.list[i].price+'" data-value3="'+data.list[i].serviceEnable+'" data-value4="'+data.list[i].expensesType+'" >'
        						+ data.list[i].price //实际应缴
        						+ '</td>'
        						+ '<input type="hidden" name="payCompList['+i+'].payValue" value="'+ data.list[i].price +'"/>'//用来存储产品应缴值
        		...
        		//拼接最后的总计，优惠，折扣，合计
        		 //循环结束，拼接合计
          		 $('#'+parentId).append('<tr id="hjDivCTB">'
           	            + ' <td>合计</td>'
           	            + '<td id="hjYJCTB">0</td>' //合计产品应缴
           	            + '<td id="hjpriceCTB">0</td>' //合计实际应缴
           	            + '<td class="sjhj" id="sjhjCTB">0</td>' //合计实缴
           	            + ' <td><div class="payment">'
           	            + '<div class="col-sm-4">'
           	            + '</div>'
           	            + '<div class="col-sm-5">'
        	        	...
        	        	if(data3.status=='success'){
        	        		...
        	        		for(var x=0; x<data3.serviceList.length; x++) {
        	        			...
        	        			if(data3.serviceList[x].isRequired=="1") {
        	        				...
        	        				tempStr += '<div class="checkbox col-sm-2 col-lg-2 no-padding-right no-padding-left">'
        	                			+ '	 <label>'
        	        					+ '	    <input type="checkbox" onclick="serviceCliCTB(this)" name="service" class="service" data-codeid="'+data3.serviceList[x].expensesTypeId+'" data-required="'+data3.serviceList[x].isRequired+'"'
        	        					+ '            data-money="'+data3.serviceList[x].money+'" data-moneyline="'+data3.serviceList[x].moneyLine+'" data-enable="'+data3.serviceList[x].serviceEnable+'"' 
        	        					+ '            data-code="'+data3.serviceList[x].expensesTypeCode+'" data-name="'+data3.serviceList[x].expensesTypeName+'" data-type="'+data3.serviceList[x].expensesType+'"'
        	        					+ '			   data-value="'+data3.serviceList[x].productServiceId+'">'
        	        					+ '		<span class="text">'+data3.serviceList[x].productServiceName+'</span>'		
        	        		...
        	        		tempStr += '</div>';
        	        		$("#branchSchoolIdCTB").parent().parent().after(tempStr);
        	        		
        	        		//遍历页面上必选的服务，把服务费用拼接到后面
        	        		$('.addProject').find("input[type='checkbox'].service").each(function(i,e){
...
function zkCalcuCTB() {
	var zkMoney = 0;//折扣金额
	$('#addPayInfo').find('tr.appendZK').each(function(index,obj) {
		zkMoney += Number($(obj).find("td.zkprice").text()) ;//统计折扣总额
	});
	
	//计算每行费用抵扣折扣结果，并且最终金额不能小于该种费用金额下限
	 $('#addPayInfo').find('tr.appendDiv').each(function (i,e) {
...
//费用列表计算合计
function hhejiCTB() {
//    var zj = $('#hjYJ').text();//合计产品应缴
//    var hj = $('#hjprice').text();//合计实际应缴
    //var yh = 0;//优惠
    var sjhj = 0;//实缴合计
    var cfhj = 0;//欠费合计
    var lss = 0;//每行实缴费用
    ...
    $('#addPayInfo').find('tr.appendDiv').each(function (i,e) {
        ...
        //剔除实缴合计的td对象
        if ($(jlid).attr('id') != "sjhjCTB" ) {
            lss = jlid.text();
            sjhj = sjhj + eval((lss != null && lss != '') ? lss : 0);
        }
        if ($(cfid).attr('id') != "cfhjCTB") {
            lss = cfid.text();
        ...
        //剔除实缴合计的td对象
        if ($(proYJtd).attr('id') != "hjYJCTB" ) {
            lss = proYJtd.text();
            proYJhj = proYJhj + eval((lss != null && lss != '') ? lss : 0);
        }
        if ($(sjYJtd).attr('id') != "hjpriceCTB") {
            lss = sjYJtd.text();
            sjYJhj = sjYJhj + eval((lss != null && lss != '') ? lss : 0);
        }
    })
//    var hj = eval(zj);
    $('#hjYJCTB').text(proYJhj);//合计产品应缴
    $('#hjpriceCTB').text(sjYJhj);//合计实际应缴
    $('#sjhjCTB').text(sjhj);//合计实缴
    $('#cfhjCTB').text(cfhj);//合计欠费结果
    //欠费合计-控制下次缴费时间的展示
    if (cfhj > 0) {
        $('#addPayTr').show();
    } else {
 ...
 * @returns
 */
function activtyCodeCTB(e) {
	var status = $("#status2").val();
    if (e.value.length == 8) {
    	//先判断是否有子产品，如果有，还要判断是否与子产品填写的优惠码相同
    	var childCodeObj = $("#childYhInputCTB");
    	if(childCodeObj!=null && typeof(childCodeObj)!="undefined") {
    		//如果有子产品优惠码对象，得到子产品的优惠码内容
    		var childCode = $("#childYhInputCTB").val();
    		//判断是否与子产品填写的优惠码相同
    		var payCode =  $(e).parent().find("input.yhInput").val();
    		if(payCode==childCode) {
    			toastr.error("子产品已经填写了该优惠码");
                //为避免本优惠码属于独立优惠，还需要做如下操作
            	//取消折扣下拉选失效
                $("#zkSelectCTB").prop("disabled",false);
        		$('#zkSelectCTB').trigger('chosen:updated');
        		//取消积分下拉选失效
        		$("#jfSelectCTB").prop("disabled",false);
        		$('#jfSelectCTB').trigger('chosen:updated');
              //发生错误后要将实缴金额制为空
                $(e).parent("input.yhqprice").val(0);//优惠金额
                $(e).parent().find("input.yhInput").val(null);//如果优惠码无效，优惠码输入框制空
                //hheji();//实时合计
            	//如果之前输入的优惠码有效，此次调用优惠折扣计算方法用来刷新页面优惠效果，取消上次的优惠码效果
                FlushYHZKCTB();//清除之前的优惠码的效果,更新页面金额
    		}
    	}
                    ...
                    if (data[0].useStatus == "1") {
                      ...
                      //发生错误后要将实缴金额制为空
                       ...
                       // toastr.success("该优惠码可以使用，优惠金额：" + data[0].amount);
                    	$(e).parent().find("input.yhqprice").val(data[0].amount);//优惠金额
                    	//订座和前面四种状态都是调用activtyCode方法，但是验证方法不一样
                    	beforeYHCTB(e);//待沟通，已沟通，预约，上门四种状态的验证方法
                        //hheji();//实时合计
                    }
                } else {
		...
		$("#jfSelectCTB").prop("disabled",false);
		$('#jfSelectCTB').trigger('chosen:updated');
    	$(e).parent().find("input.yhqprice").val(0);//如果优惠码长度不为8位，优惠金额还原为0
    	$(e).parent().find("input.yhInput").val(null);//如果优惠码长度不为8位，优惠码输入框制空
    	//如果之前输入的优惠码有效，此次调用优惠折扣计算方法用来刷新页面优惠效果，取消上次的优惠码效果
    	debugger;
    	FlushYHZKCTB();//清除之前的优惠码的效果,更新页面金额
    }
...
function beforeYHCTB(obj) {
	...
	if(isMulti=="0") {//独立
		//使折扣下拉选失效,并且使选项清空
		$("#zkSelectCTB").val('');
		$("#zkSelectCTB").prop("disabled",true);
		$('#zkSelectCTB').trigger('chosen:updated');
		//使积分下拉选失效,并且使选项清空
		$("#jfSelectCTB").val('');
		$("#jfSelectCTB").prop("disabled",true);
		$('#jfSelectCTB').trigger('chosen:updated');
...
//		toastr.error("没有收益类型费用，不能使用优惠");
		...
		$("#addPayInfo .yhInput").each(function(index,obj){
			$(obj).val(null);
	...
	if(serviceEnable=='1') {//正常
		yhCalcuNewCTB(obj,firstObj);//调用新版优惠折扣算法-只适用于正常状态（下限无效，统一为0）
	} else {//serviceEnable=='0' 锁定
		 
		yhCalcuCTB(obj,firstObj);//调用以前的优惠折扣算法-只适用于锁定状态（下限有效）
...
function yhCalcuNewCTB(obj,firstObj) {
	var trObj = $(firstObj);
   ...
   //得到优惠码的下限
	...
	trObj.find('td').eq(4).html('<div class="payment addPayment">'
	        ...
	        + '  </div>'
	        + '  <div class="col-sm-5">'
	        + '     <input class="form-control zjsjflag" ondblclick="dbclickCTB(this)" isat="1" onkeyup="sshjCTB(this)" type="text" placeholder="0" >'
	        + '	    <input type="hidden" name="payList['+0+'][payValue&Form]" class="payValueForm" value=""/>'
	        + ' </div>'
	        + '<div class="col-sm-3">'
	        + '<i onclick="addRowPayCTB(this)" data-index="'+0+'" class="fa  fa-plus-circle payment-btn blue control-label"></i>'
	        + '</div>'
	        + '</div>');//还原支付方式
	
     //计算总计，合计的应缴，实缴，欠费
     //yjCalcu();
	FlushYHZKCTB();//实际更新页面金额
}

//用于锁定状态
//优惠部分复选框单击事件-优惠部分的计算，只将金额计算到第一个费用上，不能超过金额下限
function yhCalcuCTB(obj,firstObj) {
	var trObj = $(firstObj);
   ...
   //得到优惠码的下限
	...
	trObj.find('td').eq(4).html('<div class="payment addPayment">'
	        ...
	        + ' </div>'
	        + '<div class="col-sm-3">'
	        + '<i onclick="addRowPayCTB(this)" data-index="'+indexCount+'" class="fa  fa-plus-circle payment-btn blue control-label"></i>'
	        + '</div>'
	        + '</div>');//还原支付方式
     //计算总计，合计的应缴，实缴，欠费
     //yjCalcu();
...
function yjCalcuCTB() {
	...
	$("#zjpriceCTB").text(zjValue);
	//计算合计应缴
	$("#hjpriceCTB").text(zjValue);
	...
	$("#zjsjCTB").text(zjSjValue);
	//计算合计实缴
	$("#sjhjCTB").text(zjSjValue);
	...
	$("#zjcfCTB").text(zjcfValue);
	//计算合计实缴
	$("#cfhjCTB").text(zjcfValue);
}

//折扣下拉选change事件
function zkChangeCTB(obj) {
	...
	if(isMulti=="0") {//独立
		...
		$('#jfSelectCTB').trigger('chosen:updated');
		//清空优惠码输入框，并使优惠码输入框失效
		$(".addProject .yhInput").each(function(index,obj){
			$(obj).val('');
			$(obj).prop("disabled",true);
		});
//		$(".yhInput").val('');
//		$(".yhInput").prop("disabled",true);
	...
	var newValue = $("#zkSelectCTB").val();
	$("#zkSelectCTB").val('');//优惠不可用，将该选中优惠的值制为空
	$('#zkSelectCTB').trigger('chosen:updated');
	FlushYHZKCTB();//消除之前选中的折扣的效果-更新页面
	//再将本次选中的值重新设置回去（此时的页面上的金额就是正常的金额）
	$("#zkSelectCTB").val(newValue);
	$('#zkSelectCTB').trigger('chosen:updated');
...
//	toastr.error("更新优惠");
	...
	$(".addPayFees tr.appendDiv").each(function(index,obj){
		...
		if(Number(yjValue)<Number(proValue)) {
			//当咨询量处于订座状态时，此时的实缴和支付方式中还有订座费,订座费不管费用类型，只算在第一种费用上
			if($(obj).hasClass("dingZuo")) {
	...
	//根据页面已选中的折扣更新实际应缴金额
	//先得到页面选中的折扣
	var zkValue = $("#zkSelectCTB").val();
	if(zkValue!=null && zkValue!='') {//折扣有可用值选中
		 
		//得到该折扣活动的code_id集合
		var epId = $("#zkSelectCTB").find(":selected").data("value");
		epId = epId.toString();//确保得到的是字符串
		if(epId=='' || epId==null) {//如果epId为空串
			var epIdArr = new Array();
		} else if(epId.indexOf(",")==-1) {//如果epId集合中只有一个code_id
		...
		$(".addPayFees tr.appendDiv").each(function(i,e){
			...
			for(var index in epIdArr) {
				if(codeId==epIdArr[index]) {//当前费用种类的id在code_id集合中，更新该费用的实际应缴和欠费
					...
					var proValue = $(e).find("td").eq(1).text();
					//得到当前折扣比例
					var discount = $("#zkSelectCTB").find(":selected").data("discount");
					//得到当前实际应缴值
					var yjValue = $(e).find("td").eq(2).text();
					//得到当前折扣金额
					var zkValue = Number(proValue)*(1-discount);
					//更新实际应缴金额
					...
					//设置折扣活动id
					$(e).find("input.discount").val($("#zkSelectCTB").val());
					//设置折扣金额
	...
	if(yhValue!=null && yhValue!='') {//优惠码有可用值填写
		...
		//得到优惠金额
		var yhMoney = Number($('#yhqpriceCTB').val());//优惠金额
		//得到实际应缴金额
		...
		//设置优惠码活动id
		$(firstObj).find("input.activityCode").val($("#yhInputCTB").val());
		//设置优惠码金额
		$(firstObj).find("input.activityValue").val($("#yhqpriceCTB").val());
	}
	//页面选中的积分活动-这个必须放到报名时做，因为支付金额只有最后报名时才会确定
	
	//再次更新页面上的实缴金额和支付方式，将页面上所有使用了优惠活动的费用（实际应缴小于产品应缴）
	//的实缴，支付方式还原（应为有可能第一次输入金额没有选优惠，此时产品应缴=实际应缴，然后有选择了优惠会有bug(实缴和支付方式没有清空)，所有这里要再处理一下）
	$(".addPayFees tr.appendDiv").each(function(index,obj){
		//得到产品应缴
		var proValue = $(obj).find("td").eq(1).text();
		//得到实际应缴
...
function serviceCliCTB(obj) {
	...
	var indexCount = $("#addPayInfo").find("tr.appendDiv").length-$("#addPayInfo").find("tr.zhiChu").length;
	...
	$('#hjDivCTB').before('<tr class="appendDiv" id="bao'+service+'">'
			+ ' <td>' + name + '</td>'
			+ '<td>' 
			+  price //产品应缴
			+ '</td>'
			+ '<td data-id="' + code + '" data-value="'+moneyLine+'" data-value2="'+price+'" data-value3="'+serviceEnable+'" data-value4="'+expensesType+'" >'
			+  price //实际应缴
			+ '</td>'
			...
			//下面是优惠部分参数
			+ '<input type="hidden" name="payCompList['+indexCount+'].activityCode" class="activityCode" value=""/>'//优惠码id
			+ '<input type="hidden" name="payCompList['+indexCount+'].activityValue" class="activityValue" value=""/>'//优惠码金额
			+ '<input type="hidden" name="payCompList['+indexCount+'].discount" class="discount" value=""/>'//折扣id
			+ '<input type="hidden" name="payCompList['+indexCount+'].discountValue" class="discountValue" value=""/>'//折扣金额
			+ '<input type="hidden" name="payCompList['+indexCount+'].integral" class="integral" value=""/>'//积分id
			+ '<input type="hidden" name="payCompList['+indexCount+'].integralValue" class="integralValue" value=""/>'//积分数值
			//所属的服务的id
