//构造缴费信息-拼接收益费用
function appendPayDiv(examTimeId, parentId, productId) {
        	        ...
        	        "success": function (data3) {
        	        	if(data3.status=='success'){
        	        		...
        	        		$('.project').find("input[type='checkbox'].service").each(function(i,e){
        	        		 ...
        	        		 //订座费和缴费明细不能同时展示
        	          		if($("#dztr").is(":visible")) {
        	          			$('#'+parentId).find("tr").each(function(i,e){
        	              			$(e).hide();
        	              		});
        	          		} else {
        	          			$('#'+parentId).find("tr").each(function(i,e){
...
//    var hj = $('#hjprice').text();//合计实际应缴
    ...
    $('#appendPayBody').find('tr.appendDiv').each(function (i,e) {
        ...
        //剔除实缴合计的td对象
        if ($(proYJtd).attr('id') != "hjYJ" ) {
            lss = proYJtd.text();
            proYJhj = proYJhj + eval((lss != null && lss != '') ? lss : 0);
        }
        if ($(sjYJtd).attr('id') != "hjprice") {
            lss = sjYJtd.text();
            sjYJhj = sjYJhj + eval((lss != null && lss != '') ? lss : 0);
        }
    })
//    var hj = eval(zj);
    $('#hjYJ').text(proYJhj);//合计产品应缴
    $('#hjprice').text(sjYJhj);//合计实际应缴
    $('#sjhj').text(sjhj);//合计实缴
    $('#cfhj').text(cfhj);//合计欠费结果
    //欠费合计-控制下次缴费时间的展示
    if (cfhj > 0) {
        $('#appendPayTr').show();
    } else {
...
function addRowPay(e) {
	...
	if($(e).hasClass("zhiChu")) {//是支出类型费用
		 $(e).parent().parent().parent().append('<div class="payment addPayment">'
			        ...
			        + '  </div>'
			        + '  <div class="col-sm-5">'
			        + '     <input ondblclick="dbclick(this)" class="form-control zjsjflag" isat="1" onkeyup="sshj(this)" type="text" placeholder="0" >'
...
//	
//	var td = $(e).parents('tr').find('td').eq(2);//每行实际应缴部分td对象
	//td = td[0].id; 
//	if(td=='appendPayTd1'){//如果该td是第一种费用（排在最前面）
//		var price = $(e).parents('tr').children('td').eq(2).text();//每行实际应缴部分金额
//		   if(dprice!=null){//如果订座费不为空
//			   $(e).parents('tr').children('td').eq(3).text(price);//报名费部分实缴金额
//			   $(e).val(price);//报名部分支付方式当前操作框金额-展示的时候还是按照全额展示，后台提交时再去掉报名费
//		   }else{
//			   //每行实缴金额
//			   $(e).parents('tr').children('td').eq(3).text($(e).parents('tr').children('td').eq(2).text());
//			   //每行支付方式当前操作框金额
//			   $(e).val($(e).parents('tr').children('td').eq(2).text());
//		   }
//	}else{//如果该td不属于报名费
//		//每行实缴部分金额
//		  $(e).parents('tr').children('td').eq(3).text($(e).parents('tr').children('td').eq(2).text());
                    ...
                    if (data[0].useStatus == "1") {
                      ...
                      //发生错误后要将实缴金额制为空
                       ...
                       // toastr.success("该优惠码可以使用，优惠金额：" + data[0].amount);
                    	...
                    	if(status!="6") {
                    		beforeYH(e);//待沟通，已沟通，预约，上门四种状态的验证方法
                    	} else {
                    		beforeYHZK2(e);//订座状态的验证方法
                    	}
                        //hheji();//实时合计
                    }
                } else {
...
//		return;
	...
	if(firstObj) {
		...
		$(".yhInput").each(function(index,obj){
			$(obj).val(null);
	...
	if(serviceEnable=='1') {//正常
		yhCalcuNew(obj,firstObj);//调用新版优惠折扣算法-只适用于正常状态（下限无效，统一为0）
	} else {//serviceEnable=='0' 锁定
		 
		yhCalcu(obj,firstObj);//调用以前的优惠折扣算法-只适用于锁定状态（下限有效）
...
function yhCalcuNew(obj,firstObj) {
	var trObj = $(firstObj);
   ...
   //得到优惠码的下限
    ...
    if(Number(preferential)<0) {
    	...
    	if($(trObj).hasClass("dingZuo")) {//判断当咨询量处于订座状态时
			if(yjValue-yhMoney<$("#dPrice").val()) {//如果减去优惠后，实际应缴小于订座费已缴费
				//取消折扣下拉选失效
				$("#zkSelect").prop("disabled",false);
				$('#zkSelect').trigger('chosen:updated');
				//取消积分下拉选失效
...
function yhCalcu(obj,firstObj) {
   ...
   //得到优惠码的下限
	...
	trObj.find('td').eq(4).html('<div class="payment addPayment">'
	        ...
	        + ' </div>'
	        + '<div class="col-sm-2">'
	        + '<i onclick="addRowPay(this)" data-index="'+indexCount+'" class="fa  fa-plus-circle payment-btn blue control-label"></i>'
	        + '</div>'
	        + '</div>');//还原支付方式
     //计算总计，合计的应缴，实缴，欠费
     //yjCalcu();
...
//		$(".yhInput").prop("disabled",true);
	...
	var newValue = $("#zkSelect").val();
	$("#zkSelect").val('');//优惠不可用，将该选中优惠的值制为空
	$('#zkSelect').trigger('chosen:updated');
	FlushYHZK();//消除之前选中的折扣的效果-更新页面
	//再将本次选中的值重新设置回去（此时的页面上的金额就是正常的金额）
	$("#zkSelect").val(newValue);
	$('#zkSelect').trigger('chosen:updated');
			...
			if(codeId==epIdArr[index]) {//当前费用种类的id在code_id集合中，该费用可以使用，折扣
				...
				if(serviceEnable=="0") {//如果为锁定，下限有效
					...
					if(yjValue-zkValue<moneyLine) {//如果减去折扣后，实际应缴小于下限
						...
						if($(e).hasClass("dingZuo")) {//判断当咨询量处于订座状态时
							if(yjValue-zkValue<$("#dPrice").val()) {//如果减去折扣后，实际应缴小于下限
								$("#zkSelect").val('');//优惠不可用，将该选中优惠的值制为空
								$('#zkSelect').trigger('chosen:updated');
								toastr.error($(e).find("td").eq(0).text()+"实际应缴值不小于下限，但是小于订座费已缴费");
							}
						}
					}
					...
					if(yjValue-zkValue<0) {//如果减去折扣后，实际应缴小于下限
						...
						if($(e).hasClass("dingZuo")) {//判断当咨询量处于订座状态时
							if(yjValue-zkValue<$("#dPrice").val()) {//如果减去折扣后，实际应缴小于下限
								$("#zkSelect").val('');//优惠不可用，将该选中优惠的值制为空
								$('#zkSelect').trigger('chosen:updated');
								toastr.error($(e).find("td").eq(0).text()+"实际应缴值不小于0，但是小于订座费已缴费");
							}
						}
					}
				}
...
//	toastr.error("更新优惠");
	...
	$(".projectPayFees tr.appendDiv").each(function(index,obj){
		//得到产品应缴
		...
		if(Number(yjValue)<Number(proValue)) {
			...
			if($(obj).hasClass("dingZuo")) {
				...
				//在还原实缴——设置为订座费
				$(obj).find("td").eq(3).text($("#dPrice").val());
				
				var str = '<div class="payment">'
					+ '<div class="col-sm-5">'
					...
					+ '</div>'
					+ '<div class="col-sm-5">'
					+ '<input class="form-control zjsjflag" value="'+$("#dPrice").val()+'" type="text" disabled>'
					+ '</div>'
					+ '<div class="col-sm-2">'
					+ '</div>'
					+ '</div>'
					...
					+ '</div>';
				//支付方式栏清空
				$(obj).find('td').eq(4).html(str);//还原支付方式
				//最后还原欠费-实际应缴-实缴（此时还是订座费）
				$(obj).find("td").eq(5).text(proValue-$("#dPrice").val());
			} else {
				//先还原实际应缴
				$(obj).find("td").eq(2).text(proValue);
				//在还原实缴——值为0
				...
				//接着还原支付方式列
				//支付方式栏清空
				var str = '<div class="payment addPayment">'
			        + ' <div class="col-sm-5" >'
			        + '     <select class="form-control">'
			        + '		<option value="1">现金</option>'
			        + '        <option value="2">刷卡</option>'
					...
					//+ '     <input class="form-control zjsjflag" sign="1" value="'+dPrice+'" type="text" placeholder="0" >'
//					+ '     <input ondblclick="dbclick2(this)" class="form-control zjsjflag" onkeyup="sshj(this)" value="0" type="text" placeholder="0" >';
					if($(obj).hasClass("zhiChu")) {
						str += '	    <input type="hidden" name="payZhiChuList[0][payValue&Form]" class="payValueForm" value=""/>';
					} else {
						str += '	    <input type="hidden" name="payList[0][payValue&Form]" class="payValueForm" value=""/>';
					}
					str +=  ' </div>'
			        + '<div class="col-sm-2">';
			        if($(obj).hasClass("zhiChu")) {
			        	str += '<i onclick="addRowPay(this)" data-index="'+indexCount+'" class="fa  fa-plus-circle payment-btn blue control-label zhiChu"></i>';
			        } else {
			        	str += '<i onclick="addRowPay(this)" data-index="'+indexCount+'" class="fa  fa-plus-circle payment-btn blue control-label"></i>';
			        }
			        str += '</div>'
			        + '</div>';
				$(obj).find('td').eq(4).html(str);//还原支付方式
	...
	//根据页面已选中的折扣更新实际应缴金额
	//先得到页面选中的折扣
	var zkValue = $("#zkSelect").val();
	if(zkValue!=null && zkValue!='') {//折扣有可用值选中
		 
		//得到该折扣活动的code_id集合
		var epId = $("#zkSelect").find(":selected").data("value");
		epId = epId.toString();//确保得到的是字符串
		if(epId=='' || epId==null) {//如果epId为空串
			var epIdArr = new Array();
		} else if(epId.indexOf(",")==-1) {//如果epId集合中只有一个code_id
		...
		$(".projectPayFees tr.appendDiv").each(function(i,e){
			...
			for(var index in epIdArr) {
				if(codeId==epIdArr[index]) {//当前费用种类的id在code_id集合中，更新该费用的实际应缴和欠费
					...
					var proValue = $(e).find("td").eq(1).text();
					//得到当前折扣比例
					var discount = $("#zkSelect").find(":selected").data("discount");
					//得到当前实际应缴值
					var yjValue = $(e).find("td").eq(2).text();
					//得到当前折扣金额
					...
					//设置折扣活动id
					$(e).find("input.discount").val($("#zkSelect").val());
					//设置折扣金额
	...
	if(yhValue!=null && yhValue!='') {//优惠码有可用值填写
		...
		//得到优惠金额
		var yhMoney = Number($('#yhqprice').val());//优惠金额
		//得到实际应缴金额
		...
		//设置优惠码活动id
		$(firstObj).find("input.activityCode").val($("#yhInput").val());
		//设置优惠码金额
		$(firstObj).find("input.activityValue").val($("#yhqprice").val());
	}
	...
	$(".projectPayFees tr.appendDiv").each(function(index,obj){
		...
		if(Number(yjValue)<Number(proValue)) {
			...
			if($(obj).hasClass("dingZuo")) {
				...
				$(obj).find("td").eq(3).text($("#dPrice").val());
				//支付方式栏清空
				$(obj).find('td').eq(4).html('<div class="payment">'
						+ '<div class="col-sm-5">'
						+ ' <select class="form-control" disabled>'
						+ '		<option selected>订座费已缴费</option>'
						+ ' </select>'
						...
						+ '</div>'
						+ '</div>');//还原支付方式
				//最后还原欠费=实际应缴(优惠后)-实缴（此时还是订座费）
				$(obj).find("td").eq(5).text($(obj).find("td").eq(2).text()-$("#dPrice").val());
			} else {
				//在还原实缴——值为0
				$(obj).find("td").eq(3).text(0);
				//得到之前记录的索引
				...
				var str = '<div class="payment addPayment">'
			        ...
			        + '     <input class="form-control zjsjflag" ondblclick="dbclick(this)" isat="1" onkeyup="sshj(this)" type="text" placeholder="0" >';
			        if($(obj).hasClass("zhiChu")) {
						str += '	    <input type="hidden" name="payZhiChuList['+indexCount+'][payValue&Form]" class="payValueForm" value=""/>';
					} else {
						str += '	    <input type="hidden" name="payList['+indexCount+'][payValue&Form]" class="payValueForm" value=""/>';
					}
			        str += ' </div>'
			        + '<div class="col-sm-2">';
			        if($(obj).hasClass("zhiChu")) {
...
function serviceCli(obj) {
	...
	var indexCount = $("#appendPayBody").find("tr.appendDiv").length-$("#appendPayBody").find("tr.zhiChu").length;
	...
	$('#hjDiv').before('<tr class="appendDiv" id="'+service+'">'
			+ ' <td>' + name + '</td>'
			+ '<td>' 
			+  price //产品应缴
			+ '</td>'
